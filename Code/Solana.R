...
data_solana<-read.csv("/Users/sabrinachowdhury/Desktop/BigData_Spring2022/Project/Data_Set/coin_Solana.csv")
data_solana$avg_price <- rowMeans(data_solana[,5:6], na.rm=TRUE,dims=1)

data_xts <- xts(x=data_solana$avg_price, order.by=as.Date(data_solana$Date,format =c("%Y-%m-%d %H:%M:%S")),frequency=365)
...
# plot(f_ets,main="stellar prediction using ETS",ylab="Solana price (per unit) in $")# xlim=c(2020,2022))
# #plot(f_ets,main="stellar prediction using ETS",ylab="stellar price (per unit) in $", xlim=c(2018.9,2019.9), ylim=c(0,1000))
# write.csv(f_ets,"/Users/sabrinachowdhury/Desktop/BigData_Spring2022/Project/Data_Set/ets_solana_forecast.csv")
# 
# 
# # arima model
...
plot(fcast,main="stellar prediction using ARIMA",ylab="Solana price (per unit) in $")# xlim=c(2018.9,2019.9), ylim=c(0,1000))
write.csv(fcast,"/Users/sabrinachowdhury/Desktop/BigData_Spring2022/Project/Data_Set/Arima_solana_forecast.csv")
...
# #creating data frame
# data <- as.data.frame(t(matrix(data_xts, 100)))
# 
