...
ada=pd.read_csv("/Users/sabrinachowdhury/Desktop/BigData_Spring2022/Project/Data_Set/coin_Dogecoin.csv",index_col=0,usecols=[3,5])
ada.rename(columns={"Low":"Price"},inplace = True)
#ada.iloc[2950:2991]
ada.head()        
...
ada['Prediction'] = ada['Price'].shift(-prediction_days)

# Create independent data set
# Convert the dataframe to a numpy array and drop prediction column
X1 = np.array(ada.drop(['Prediction'], 1))

# Remove the last n rows where n is the prediction_days
X1 = X1[:len(ada)-prediction_days]

# Create a depedent data set
# Convert the dataframe to a numpy array and drop prediction column
y1 = np.array(ada['Prediction'])
...
# for ada
prediction_days_array1 = (ada.drop(['Prediction'],1))[-prediction_days:]
...
new_predictions = pd.DataFrame([svm_prediction1[:730], y1_test], index=['ADA','ADA_ACTUAL']) 
...
plt.subplot(3,2,1)
sns.lineplot(x = lisst, y = new_predictions['ADA'][:730], marker = 'o',color='orange' )
plt.title("ADA Prediction Prices vs Days")
plt.xlabel("Days")
plt.ylabel("Predicted Price")

plt.subplot(3,2,2)
new_predictions['ADA_ACTUAL'][:730].plot(marker='o',color='blue')
...
new_list=pd.DataFrame(new_predictions['ADA'][:730]*7)

new_list.to_csv("/Users/sabrinachowdhury/Desktop/BigData_Spring2022/Project/Data_Set/svm_doge_pred.csv")
