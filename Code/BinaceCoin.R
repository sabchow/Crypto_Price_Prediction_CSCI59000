...
data_binanceCoin<-read.csv("/Users/sabrinachowdhury/Desktop/BigData_Spring2022/Project/Data_Set/coin_BinanceCoin.csv")
data_binanceCoin$avg_price <- rowMeans(data_binanceCoin[,5:6], na.rm=TRUE,dims=1)

data_xts <- xts(x=data_binanceCoin$avg_price, order.by=as.Date(data_binanceCoin$Date,format =c("%Y-%m-%d %H:%M:%S")),frequency=365)
...
# plot(f_ets,main="binanceCoin prediction using ETS",ylab="binanceCoin price (per unit) in $")# xlim=c(2020,2022))
# #plot(f_ets,main="binanceCoin prediction using ETS",ylab="binanceCoin price (per unit) in $", xlim=c(2018.9,2019.9), ylim=c(0,1000))
# write.csv(f_ets,"/Users/sabrinachowdhury/Desktop/BigData_Spring2022/Project/Data_Set/ets_bnb_forecast.csv")
# 
# 
# # arima model
...
plot(fcast,main="binanceCoin prediction using ARIMA",ylab="binanceCoin price (per unit) in $")# xlim=c(2018.9,2019.9), ylim=c(0,1000))
write.csv(fcast$upper,"/Users/sabrinachowdhury/Desktop/BigData_Spring2022/Project/Data_Set/arima_bnb_forecast.csv")
...
# #creating data frame
# data <- as.data.frame(t(matrix(data_xts, 365)))
# 
